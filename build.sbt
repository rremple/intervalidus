...
lazy val latestScala3Nightly: Option[String] =
  val m = "https://repo.scala-lang.org/artifactory/maven-nightlies/org/scala-lang/scala3-compiler_3/maven-metadata.xml"
  try
    val url = URI.create(m).toURL
    val builder = DocumentBuilderFactory.newInstance().newDocumentBuilder()
    val doc = Using.resource(url.openStream())(builder.parse)
    val nodes = doc.getElementsByTagName("latest")
    if nodes.getLength == 0 then
      println(s"Can't find latest: no metadata")
      None
    else
    ...
    case e: Exception =>
      println(s"Failed to find latest: $e")
      None
...
lazy val scalaVersionString = {
  val defaultScalaVersion = "3.9.0"
  val useScalaVersion = sys.env.getOrElse("SCALA_VERSION", defaultScalaVersion)
  println(s"Scala $useScalaVersion")
  useScalaVersion
}
...
versionPolicyIntention := Compatibility.BinaryCompatible // None // as a stopgap
...
homepage := Some(uri("https://github.com/rremple/intervalidus"))
...
// Gets ProjectReference from Project implicitly
...
lazy val `intervalidus-pickle` = project
  .in(file("json/common"))
  .dependsOn(core)
...
lazy val `intervalidus-upickle` = project
...
lazy val `intervalidus-circe` = project
  .in(file("json/circe"))
...
lazy val `intervalidus-play` = project
  .in(file("json/play"))
  .dependsOn(core, `intervalidus-pickle` % "test->test")
...
lazy val `intervalidus-tinyrule` = project
  .in(file("sidequests/tinyrule"))
  .settings(commonPublishSettings("intervalidus-tinyrule"))

lazy val `intervalidus-examples` = project
  .in(file("examples"))
  .disablePlugins(MimaPlugin)
