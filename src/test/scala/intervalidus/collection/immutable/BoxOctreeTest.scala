...
class BoxOctreeTest extends AnyFunSuite with Matchers:
  test("Immutable: BoxOctree methods"):
    ...
    val tree = BoxOctree[String](boundary)
      .addOne(Box3D(Coordinate3D(3, 3, 1), Coordinate3D(5, 5, 2)) -> "one") // right upper front
      .addOne(Box3D(Coordinate3D(-1, 1, 1), Coordinate3D(3, 3, 2)) -> "two") // left and right upper front (split)
      .addOne(Box3D(Coordinate3D(1, -1, 1), Coordinate3D(3, 1, 2)) -> "three") // right lower and upper front (split)
      .addOne(Box3D(Coordinate3D(-1, -1, -1), Coordinate3D(1, 1, 1)) -> "four") // all ocrants (split)
    ...
    val leaf = BoxOctreeLeaf[String](boundary, depth = 0, capacity = 1, depthLimit = 1)
      .addOne(Box3D(Coordinate3D(3, 3, 1), Coordinate3D(5, 5, 2)) -> "one")
    val leafCopy = leaf.copy
    leaf.toIterable should contain theSameElementsAs List(Box3D(Coordinate3D(3, 3, 1), Coordinate3D(5, 5, 2)) -> "one")
    ...
    val treeSplit = BoxOctree
      .from(boundary, BoxedPayload.deduplicate(tree.toIterable), 4)
      .addOne(Box3D(Coordinate3D(3, 5, 1), Coordinate3D(7, 7, 2)) -> "five") // splits right upper front
      .addOne(Box3D(Coordinate3D(5, 4, 1), Coordinate3D(7, 5, 2)) -> "six")
    ...
    val shrunkenTree = treeSplit
      .remove(BoxedPayload3D(Box3D(Coordinate3D(3, 3, 1), Coordinate3D(5, 5, 2)), "one"))
      .remove(BoxedPayload3D(Box3D(Coordinate3D(5, 4, 1), Coordinate3D(7, 5, 2)), "six"))
