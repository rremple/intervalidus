...
object BsonTransformer:
  ...
  given (bsonValueWriter: Writer[BsonValue]) => Writer[BsonDocument] = bsonValueWriter.narrow

  given (bsonValueReader: Reader[BsonValue]) => Reader[BsonDocument] = bsonValueReader.map(_.asDocument())
