...
object BillingIn2D extends Billing:
  ...
  private def oneAtATime(): Unit =
    ...
    val johnUpdate1 =
      customerTiers0(Customer.john.id) + ((intervalFrom(Feb / 1) x intervalFrom(Jan / 15)) -> Tier.basic)
    // Also on January 15, Jane signs up for a premium plan starting March 1
    val janeUpdate1 =
      customerTiers0(Customer.jane.id) + ((intervalFrom(Mar / 1) x intervalFrom(Jan / 15)) -> Tier.premium)
    ...
    val johnUpdate2 =
      customerTiers1(Customer.john.id) + ((intervalFrom(Apr / 1) x intervalFrom(Mar / 17)) -> Tier.premium)
    // On March 20, Jane terminates her subscription immediately and does not renew
    val janeUpdate2 = customerTiers1(Customer.jane.id) - (intervalFromAfter(Mar / 20) x intervalFrom(Mar / 20))
  ...
  private def allAtOnce(): Unit =
    ...
    // on March 17, John upgrades to premium starting April 1;
    // on June 28, John decides not to renew for July
    val johnUpdates = customerTiers0(Customer.john.id) ++ Seq(
      (intervalFrom(Feb / 1) x intervalFrom(Jan / 15)) -> Tier.basic,
      (intervalFrom(Apr / 1) x intervalFrom(Mar / 17)) -> Tier.premium
    ) - (intervalFromAfter(Jun / 30) x intervalFrom(Jun / 28))
    // Also on January 15, Jane signs up for a premium plan starting March 1;
    // on March 20, Jane terminates her subscription immediately and does not renew
    val janeUpdates = customerTiers0(Customer.jane.id)
      + ((intervalFrom(Mar / 1) x intervalFrom(Jan / 15)) -> Tier.premium)
      - (intervalFromAfter(Mar / 20) x intervalFrom(Mar / 20))
