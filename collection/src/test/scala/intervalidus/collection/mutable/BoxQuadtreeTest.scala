...
class BoxQuadtreeTest extends AnyFunSuite with Matchers:
  ...
  test("Mutable: BoxTree (2D -- Quadtree) methods"):
    ...
    tree.addOne(Box(coordinate(3, 3), coordinate(5, 5)) -> "one") // right upper
    tree.addOne(Box(coordinate(-1, 1), coordinate(3, 3)) -> "two") // left and right upper (split)
    tree.addOne(Box(coordinate(1, -1), coordinate(3, 1)) -> "three") // right lower and upper (split)
    tree.addOne(Box(coordinate(-1, -1), coordinate(1, 1)) -> "four") // all quadrants (split)
    ...
    shrunkenTree.remove(BoxedPayload(Box(coordinate(3, 3), coordinate(5, 5)), "one"))
    shrunkenTree.remove(BoxedPayload(Box(coordinate(5, 4), coordinate(7, 5)), "six"))
    shrunkenTree.remove(BoxedPayload(Box(coordinate(5, 4), coordinate(7, 5)), "never added"))
