...
class ContinuousValueTest extends AnyFunSuite:
  ...
  test("Ops on Int Domain"):
    ...
    assert(domain(4).tupled afterStart domain(3).tupled)
    assert(open(4).tupled afterStart domain(4).tupled)
    assert(!(domain(4).tupled afterStart domain(4).tupled))
    assert(domain(4).tupled afterOrAtStart domain(4).tupled)
    assert(domain(4).tupled beforeOrAtEnd domain(4).tupled)
