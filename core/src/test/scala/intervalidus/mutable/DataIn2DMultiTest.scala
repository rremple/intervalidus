...
class DataIn2DMultiTest
  ...
  test(s"Mutable: Int DataMulti 2D collection operations"):
    extension [T](data: DataMulti[T, Dim])
      def mutate(f: DataMulti[T, Dim] => Unit): DataMulti[T, Dim] =
    ...
    assert(donut.mutate(_ ∪ hole).domain.isUniverse)
    donut.mutate(_ ∪ hole) shouldBe DataMulti(Seq(a, b, c, d).map(_ -> donutFilling) ++ Seq(e -> holeFilling))

    Seq(a, b).donutFilled.mutate(_ ∪ Seq(c, d).donutFilled) ≡≡ donut
    Seq(a, b).donutFilled.mutate(_ ∪ Seq(b, c).holeFilled) ≡≡
      DataMulti(Seq(a -> donutFilling, b -> fillingUnion, c -> holeFilling))
    Seq(a, e).donutFilled.mutate(_ ∩ Seq(e, d).holeFilled) ≡≡ Seq(e).valueFilled(fillingIntersection)

    val clipInterval = interval(-10, 10) x interval(-10, 10)
    val clippedDonutWithHole = donut.mutate(_ ∪ hole).mutate(_ ∩ clipInterval)
