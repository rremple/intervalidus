...
class DataIn4DTest extends AnyFunSuite with Matchers with DataIn4DBaseBehaviors with MutableBaseBehaviors:
  ...
  def mapf(
    d: ValidData[String, MixedDim]
  ): ValidData[String, MixedDim] =
    d.copy(
      value = d.value + "!",
  ...
  def vertical4D[T2: DiscreteValue](interval3: Interval1D[T2]): Interval.In4D[LocalDate, LocalDate, T2, Int] =
  ...
  test("Mutable: Diff actions"):
    ...
    fixture.removeByKey((intervalFrom(day(1)) x unboundedDate x intervalFrom(1) x unbounded[Int]).start)
