...
class DataAffineDiscreteTest
  ...
  testsFor(commonBehaviors("Discrete immutable"))

  testsFor(affineBehaviors("Discrete immutable"))
  ...
  test("Discrete immutable: Int DataAffine reflecting"):
    ...
    clippedDonut.reflectedAboutIn(0, 0) ≡≡ clippedDonut.displacedBy((-1, 0)) // nearly symmetric
    ...
    clippedDonut.reflectedAboutIn(0, 1) ≡≡ clippedDonut.displacedBy((1, 0)) // nearly symmetric

    // And there's nothing between 0 and 1 that you can use as the center in a discrete domain.

    clippedDonut.reflectedAboutIn(0, 1).displacedBy((1, 0)).getAll.toList shouldBe List(
    ...
    clippedDonut.reflectedAboutIn(1, 0) ≡≡ clippedDonut.displacedBy((0, -1)) // nearly symmetric

    clippedDonut.reflectedAboutIn(1, 1).displacedBy((0, 1)).getAll.toList shouldBe List(
      (intervalFrom(-10)
    ...
    clippedDonut.reflectedAbout(Domain.in2D(0, 0)) ≡≡ clippedDonut.displacedBy((-1, -1)) // nearly symmetric

    clippedDonut.reflectedAbout(Domain.in2D(1, 1)).displacedBy((1, 1)).getAll.toList shouldBe List(
  ...
  test(s"Discrete immutable: Int DataAffine scaling"):
