...
class DataIn2DVersionedTest extends AnyFunSuite with Matchers with DataIn2DVersionedBaseBehaviors:
  ...
  def newDataIn2DVersioned(
    ...
    allData.foldLeft(DataVersioned.empty[String, IntDim]): (dataIn2DVersioned, validData) =>
      dataIn2DVersioned
        .set(validData)
  ...
  // shared
  testsFor(stringLookupTests("Immutable", newDataIn2DVersioned, DataVersioned(_), DataVersioned.ofValue(_)))
  ...
  test("Immutable: Mapping, flat mapping, etc."):
    ...
    fixture7.boundingInterval shouldBe None
    assertThrows[NoSuchElementException]:
      fixture7.get
