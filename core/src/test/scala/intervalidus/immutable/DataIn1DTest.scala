...
class DataIn1DTest extends AnyFunSuite with Matchers with DataIn1DBaseBehaviors with ImmutableBaseBehaviors:
  ...
  def usingSetMany(data: IterableOnce[ValidData[String, IntDim]]): Data[String, IntDim] =
  ...
  testsFor(immutableCompressionTests[IntDim, Data[String, IntDim]](Data(_), identity))
  testsFor(immutableCompressionTests[IntDim, Data[String, IntDim]](usingBuilder, identity, "Immutable (builder)"))
