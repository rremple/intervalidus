...
class DataIn2DTest extends AnyFunSuite with Matchers with DataIn2DBaseBehaviors with ImmutableBaseBehaviors:
  ...
  )(using config: CoreConfig[MixedDim]): Assertion =
  ...
  test("Immutable: Constructors and getting data by index"):
    ...
    val fixture: Data[String, Domain.In2D[LocalDate, Int]] = mutable.Data(allData).toMutable // implicit conversion
  ...
  test("Immutable: Diff actions"):
    ...
    val fixture5 = fixture4
      ...
      .removeByKey((intervalFrom(day(1)) x intervalFrom(1)).start)
  ...
  test(s"Immutable: Int Data 2D collection operations"):
    ...
    val clippedDonutWithHole = donut.mergeMany(hole.getAll) ∩ clipInterval
