...
class DataFunctionContinuousTest extends AnyFunSuite with Matchers:
  ...
  extension (actual: (alongX: Double, alongY: Double))
    infix def shouldBeAbout(expected: (alongX: Double, alongY: Double)): Assertion = assert(
      math.abs(actual.alongX - expected.alongX) < epsilon && math.abs(actual.alongY - expected.alongY) < epsilon,
      s"\nExpected: \n$expected ± $epsilon\nActual: \n$actual\n"
    )
  ...
  test("Immutable: Construction, mapping, and collecting"):
    ...
    val mapped: DataFunction[Int, Dim2D] = surface2D.map:
    ...
    mapped(in2D(0.0, 0.0)) shouldBe 10 // Center point where all regions converge
    mapped(in2D(1.0, 1.0)) shouldBe 1 // Q1: Gaussian evaluation at (1.0, 1.0), (10 * exp(-2.0)).toInt = 1
    mapped(in2D(-1.5, 2.0)) shouldBe 10 // Q2: Flat plateau at (-1.5, 2.0)
    mapped(in2D(0.5, -0.5)) shouldBe 5 // Q3/Q4: Clamped dome inside unit circle (10 * 0.5 = 5)
    mapped(in2D(1.0, -1.0)) shouldBe 0 // Q3/Q4: Clamped dome outside unit circle

    val mappedValues = surface2D.mapValues(d => (10 * d).toInt)
    ...
    mappedValues(in2D(0.0, 0.0)) shouldBe 10 // Center point where all regions converge
    mappedValues(in2D(1.0, 1.0)) shouldBe 1 // Q1: Gaussian evaluation at (1.0, 1.0), (10 * exp(-2.0)).toInt = 1)
    mappedValues(in2D(-1.5, 2.0)) shouldBe 10 // Q2: Flat plateau at (-1.5, 2.0)
    mappedValues(in2D(0.5, -0.5)) shouldBe 5 // Q3/Q4: Clamped dome inside unit circle (10 * 0.5 = 5)
    mappedValues(in2D(1.0, -1.0)) shouldBe 0 // Q3/Q4: Clamped dome outside unit circle

    val mappedIntervals: DataFunction[Double, Dim2D] = surface2D.mapIntervals:
    ...
    val collected = surface2D.collect: // extend Q1 negative Gaussian to the whole domain
    ...
    val collectedValues = surface2D.collectValues: // drop the flat plateau and negate results
    ...
    val collectedIntervals = surface2D.collectIntervals: // extend Q1 Gaussian to the whole domain
    ...
    val flatmapped = collectedIntervals.flatMap: d => // clamp value outside a bounded interval
  ...
  test("Immutable: Continuity, diff actions, and queries"):
    ...
    val safeReciprocal = unsafeReciprocal.remove(intervalAt(0.0)) // undefined at x = 0
    ...
    unsafeReciprocal.applyDiffActions(diffActions) ≡≡ safeReciprocal
    unsafeReciprocal.syncWith(safeReciprocal) ≡≡ safeReciprocal
    ...
    val shiftedReciprocal = safeReciprocal.mapIntervals: i =>
    ...
    uncompressed.compress(reciprocalFunction) shouldBe shiftedReciprocal
    uncompressed.compressAll() shouldBe shiftedReciprocal
    uncompressed.recompressAll() shouldBe shiftedReciprocal

  test("Immutable: Dimension transforms and other geometry"):
    ...
    val clippedProfileAlongX = maxProfileAlongX ∩ interval(-1.0, 2.0)
    val clippedProfileAlongY = maxProfileAlongY intersection interval(-2.0, 1.0)
    ...
    val merged = clippedProfileAlongX.merge(clippedProfileAlongY, average)
    ...
    val mergedMany = clippedProfileAlongX.mergeMany(clippedProfileAlongY.getAll, average)
    ...
    val xor = clippedProfileAlongX △ clippedProfileAlongY
    ...
    val diff = clippedProfileAlongX \ clippedProfileAlongY
  ...
  test("Immutable: Thin tests for thin wrappers"):
    val reciprocalFunction: DomainFunction[Double, Dim1D] = _ match
      case Point.In1D(x) => 1.0 / x
      case _             => 0.0 // as x -> ∞ and -∞, 1/x -> 0
    val safeReciprocal = DataFunction.ofValue(reciprocalFunction) - intervalAt(0.0)

    val positive = safeReciprocal.filter(_.interval.contains(1.0))
    ...
    val positiveClamped = safeReciprocal + clampPositive
    positiveClamped(10.0) shouldBe 0.0
    val bothClamped = safeReciprocal ++ Seq(clampPositive, clampNegative)
    ...
    val unsafeFilled = safeReciprocal.fill(unbounded -> posInfinityConstant)
    unsafeFilled(0.0) shouldBe Double.PositiveInfinity
    unsafeFilled(1.0) shouldBe 1.0

    safeReciprocal.setIfNoConflict(intervalAt(0.0) -> posInfinityConstant) match
      case None         => fail("expected to set once")
      case Some(unsafe) =>
        unsafe.setIfNoConflict(intervalAt(0.0) -> posInfinityConstant) match
          case Some(_) => fail("didn't expect to set twice")
          case None    => succeed
        unsafe(0.0) shouldBe Double.PositiveInfinity
        val unsafeNeg = unsafe.update(intervalAt(0.0) -> negInfinityConstant)
        unsafeNeg(0.0) shouldBe Double.NegativeInfinity
        val unsafeZero = unsafe.replace(
          intervalAt(0.0) -> posInfinityConstant,
          intervalAt(0.0) -> zeroConstant
        )
        unsafeZero(0.0) shouldBe 0.0
        unsafe.replaceByKey(0.0, intervalAt(0.0) -> zeroConstant).getAt(0.0) shouldBe Some(0.0)
        unsafe.removeByKey(0.0).getAt(0.0) shouldBe None
        (unsafe -- Seq(intervalToBefore(-10.0), intervalFromAfter(10.0))).domain shouldBe
          IntervalShape.of(interval(-10.0, 10.0))
        unsafe.removeValue(posInfinityConstant) ≡≡ safeReciprocal
