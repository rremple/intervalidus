...
trait DataIn4DMultiBaseBehaviors:
  ...
  def withHorizontal[T: DiscreteValue](i: Interval1D[T]): Interval.In4D[T, T, T, T] =
    Interval.in4D[T, T, T, T](i, unbounded, unbounded, unbounded)

  def withHorizontal[T: DiscreteValue](d: Domain1D[T]): Domain.In4D[T, T, T, T] =
  ...
  def basicAndZipTests[S <: DimensionalMultiBase[String, IntDim]](
    ...
    test(s"$prefix: Basics"):
      ...
      f0
        .getByDimension(1, 15)[Domain.In3D[Int, Int, Int]]
        .getByDimension(2, 15)[Domain.In2D[Int, Int]]
        .toString shouldBe
        """|| 0 .. 4           | 5 .. 9           | 10 .. 20         | 21 .. 25         | 26 .. 30         |
           || {A} (-∞..+∞)     |
           |                   | {A,B} (-∞..+∞)   |
    ...
    test(s"$prefix: 4D-specific methods"):
      ...
      fixture1
        .getByDimension(2, 10)[Domain.In3D[Int, Int, Int]]
