...
class DiscreteIntervalTest extends AnyFunSuite with Matchers with IntervalCommonBehaviors:
  ...
  test("Discrete: Int discrete-specific interval behaviors"):
    ...
    // Points
    intervalFrom(Int.MaxValue - 1).points.toList shouldBe List(Int.MaxValue - 1, Int.MaxValue).map(domain(_))
    intervalTo(Int.MinValue + 1).points.toList shouldBe List(Int.MinValue, Int.MinValue + 1).map(domain(_))
    interval(1, 3).points.toList shouldBe List(1, 2, 3).map(domain(_))
    ...
    interval4d(1, 2, 1, 2, 1, 2, 1, 2).points.toList shouldBe List(
      (1, 1, 1, 1),
      (1, 1, 1, 2),
      (1, 1, 2, 1),
      (1, 1, 2, 2),
