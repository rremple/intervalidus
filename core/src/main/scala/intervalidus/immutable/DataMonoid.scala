...
object DataMonoid extends DimensionalMonoidBaseObject[DataMonoid]:
  ...
  extension [V: Monoid, D <: NonEmptyTuple: DomainLike](data: DimensionalBase[V, D])
    /**
      * Creates an immutable monoidal structure from a non-monoidal structure with monoidal values.
      *
      * @return
      *   A new immutable monoidal structure with the same valid values.
      */
    def asDataMonoid: DataMonoid[V, D] = new DataMonoid(data.stateCopy)(using config = data.config)

  /**
    * Automatically converts a non-monoidal structure with monoidal values to an immutable monoidal structure.
    */
...
class DataMonoid[V, D <: NonEmptyTuple: DomainLike] private (
  ...
  override def collectIntervals[S <: NonEmptyTuple: DomainLike](
    ...
    DataMonoid(collectIntervalsData(pf))(using config = altConfig).compressedUpdate()
  ...
  override def getByHeadDimension[H: DomainValueLike](domain: Domain1D[H])(using
    ...
    DataMonoid(getByHeadDimensionData(domain))(using config = altConfig).compressedUpdate()
  ...
  override def toMutable: intervalidus.mutable.DataMonoid[V, D] =
    intervalidus.mutable.DataMonoid.asDataMonoid(this)
