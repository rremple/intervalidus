...
/**
  ...
  *     the subtrees of the data structure. Intervalidus uses the ordered hashes defined on the domain value components
  *     of intervals to approximate all intervals as boxes in double space, and manages valid data associated with these
  *     boxes in box search trees, resulting in dramatically faster intersection retrieval (in most cases).
